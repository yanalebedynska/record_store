...
def bokeh_bar_chart_1(request):
    try:
        ...
        # Підготовка даних для стовпчастої діаграми
        products = df['product__name'].tolist()
        counts = df['order_count'].tolist()

        # Створення стовпчастої діаграми
        p = figure(x_range=products, height=500, width=800, title="Order Count by Product",
                   toolbar_location=None, tools="hover", tooltips="@x: @top")

        p.vbar(x='product__name', top='order_count', width=0.8, color=factor_cmap('product__name', palette=Category20c[len(df)], factors=products), source=df)
        ...
        # Логування помилки
        print(f"Error in bokeh_bar_chart: {e}")
        return JsonResponse({"error": str(e)}, status=500)
...
def bokeh_pie_chart_2(request):
    try:
        # Отримуємо всі дані через репозиторій
        repository = ReceiptItemRepository()
        queryset = repository.get_all()  # Отримуємо дані з репозиторію
        data = queryset.values('receipt_item_id', 'quantity', 'product__price')  # Обираємо потрібні поля
        df = pd.DataFrame(list(data))  # Перетворення у DataFrame

        # Перевірка, чи є дані
        if df.empty:
            return JsonResponse({"error": "No data available for pie chart"}, status=404)

        # Обчислення сумарної кількості для кожного товару
        df_grouped = df.groupby('receipt_item_id')['quantity'].sum().reset_index()
        df_grouped.rename(columns={'quantity': 'total_quantity'}, inplace=True)

        # Обчислення відсотків
        df_grouped['angle'] = df_grouped['total_quantity'] / df_grouped['total_quantity'].sum() * 2 * pi
        df_grouped['color'] = Category20c[len(df_grouped)]

        # Створення кругової діаграми
        p = figure(height=500, width=500, title="Receipt Items Distribution by Quantity",
                   toolbar_location=None, tools="hover", tooltips="@receipt_item_id: @total_quantity", x_range=(-0.5, 1.0))
...
def bokeh_bar_chart_3(request):
    try:
        ...
        queryset = repository.get_all()
        data = queryset.values('product__category', 'order_date', 'quantity', 'product__price')
        print("Fetched raw data:", list(data))
        ...
        print("Grouped data:", category_group)

        if category_group.empty:
            print("No grouped data available")
            return JsonResponse({"error": "No grouped data available"}, status=404)
        ...
        p.vbar(
            ...
            top='total_income',
            width=0.8,
            color=factor_cmap('product__category', palette=palette, factors=category_group['product__category'].tolist()),
...
def bokeh_line_chart_4(request):
    try:
        ...
        p = figure(
            ...
            toolbar_location=None,
            tools="hover",
            tooltips="@x: @y Products"
        )

        # Додавання лінії на графік
        p.line(
            x=list(range(len(df))),  # Числові індекси для осі X
            y=df['total_products'],
            line_width=2,
            color="blue",
        ...
        print(f"Error in bokeh_line_chart_suppliers: {e}")
        return JsonResponse({"error": str(e)}, status=500)
...
def bokeh_pie_chart_5(request):
    try:
        ...
        df['angle'] = df['total_products'] / df['total_products'].sum() * 2 * 3.14159
        df['color'] = Category20c[len(df)]  # Вибір кольорів із палітри
